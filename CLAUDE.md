...
- **Wallan Zoho Desk VERIFIED end-to-end (2026-08-11): T10–T12 pass** against production Zoho Desk — tickets **94505** (Book a Service), **94506** (Contact Us), **94507** (service, sparse fields + Arabic pref time), all on the "Genesis CS Department" layout with contacts auto-created. Flagged TEST in the description — **ask Wallan to delete them**. T13 confirms sales still route to CRM Leads (record `6174926000177680131` — also TEST, also to delete). Evidence: `distro_crms/integration-tests/results.json`.
- Corrections to their "Genesis API Documentation Desk - V8" doc, all verified live:
  - The inline `contact` object accepts only `firstName`/`lastName`/`email`/`phone`. Anything extra — we tried `mobile` — is a hard **422 UNPROCESSABLE_ENTITY**, not ignored.
  - Empty strings on typed custom fields are rejected, so `sendToWallanDesk` prunes empty `cf_*` keys before sending.
...
- **Possible Desk refinement (low priority):** the live Genesis Desk layout carries more custom fields than the V8 doc lists — including `cf_country`, `cf_title`, `cf_brand`, `cf_lead_form_name`, `cf_service_required`, `cf_email_optin`/`cf_sms_optin`. We currently put "UAE" in `cf_city_1` (free text); `cf_country` may be the better home, and the name prefix could go to `cf_title`. Left alone since the verified run passes — confirm field types with Wallan before changing.
