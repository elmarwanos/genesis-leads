...
- `src/backend/distributorCRM.js` — Per-country CRM fan-out (fire-and-forget, called via `setTimeout(..., 0)` so failures never block the 201 response). Routing is by `reqBody.country`:
  ...
  - **UAE → Wallan** Zoho CRM v5 Leads. OAuth2 refresh-token flow, fresh access token per call. Secrets: `wallan_zoho_client_id`, `wallan_zoho_client_secret`, `wallan_zoho_refresh_token`. Layout id `6174926000000091055` is hardcoded (org's "Standard" Leads layout — the id in Wallan's V8 doc, `6643351...`, is from a *different* Zoho org and gets rejected with INVALID_DATA; verified live 2026-07-14). UAE Book-a-Service goes to Zoho **Desk** — not implemented, docs pending from Innocean.
...
- **Zoho token caching (do before go-live):** module fetches a fresh access token per lead; Zoho rate-limits refresh grants (~10/10 min) — a lead burst would throttle. Cache the token (~55 min TTL).
...
- Snapchat sub-source: decided 2026-07-14 to leave as `"None"` (not in Wallan's picklist; Snapchat leads not flowing from our side anyway).
- Wallan Zoho Desk (UAE Book a Service) — deferred; awaiting API docs from Innocean (Mohamad Kader).
